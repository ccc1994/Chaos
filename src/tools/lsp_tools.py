...
def find_symbol_position(file_path: str, symbol_name: str) -> Optional[Position]:
    """
    在文件中查找符号首次出现的位置。
    返回 Position(line=XX, character=XX) 或者是 None。
    注意：LSP 的行号从 0 开始。
    """
    ...
    try:
        ...
        for i, line in enumerate(lines):
            ...
            if match:
                return Position(line=i, character=match.start())
    except Exception:
        pass
    return None
...
async def _get_call_hierarchy_internal(file_path: str, line: int, character: int, direction: str = "incoming") -> str:
    ...
    try:
        ...
        item = items[0]
        output = [f"符号: {item.name} ({item.kind.name if hasattr(item.kind, 'name') else item.kind})", f"位置: {item.uri.replace('file://', '')}:{item.range.start.line + 1}"]
        ...
        if direction == "incoming":
            ...
            if not calls:
                ...
                for call in calls:
                    ...
                    from_path = from_item.uri.replace("file://", "")
            ...
            if not calls:
                ...
                for call in calls:
                    to_item = call.to
                    to_path = to_item.uri.replace("file://", "")
                    ranges_str = ", ".join([f"L{r.start.line+1}" for r in call.from_ranges])
...
async def lsp_get_definition(file_path: str, symbol_name: str, line: int = -1, character: int = -1) -> str:
    ...
    """
    if line == -1:
        pos = find_symbol_position(file_path, symbol_name)
        if not pos:
            return f"在文件 {file_path} 中未找到符号 '{symbol_name}'。"
        l, c = pos.line, pos.character
    else:
